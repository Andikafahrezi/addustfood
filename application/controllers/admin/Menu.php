...
class Menu extends CI_Controller {
    ...
    public function indexd() {
        $this->load->model('Menu_model');
        $dishesh = $this->Menu_model->getMenuu();
        $data['dishesh'] = $dishesh;
        $this->load->view('admin/partials/header');
        $this->load->view('admin/menu/list_display', $data);
        $this->load->view('admin/partials/footer');
    }
    ...
    public function create_menud(){

        $this->load->helper('common_helper');
        $this->load->model('Store_model');
        $store = $this->Store_model->getStores();

        $config['upload_path']          = './public/uploads/dishesh/display/';
        $config['allowed_types']        = 'gif|jpg|png|jpeg';
        //$config['encrypt_name']         = true;
        ...
        if($this->form_validation->run() == true) {
            ...
            if(!empty($_FILES['image']['name'])){
                ...
                if($this->upload->do_upload('image')) {
                    ...
                    $this->Menu_model->created($formArray);
        
                    $this->session->set_flashdata('dish_success', 'Menu added successfully');
                    redirect(base_url(). 'admin/menu/indexd');
                    ...
                    $data['stores']= $store;
                    $this->load->view('admin/partials/header');
                    $this->load->view('admin/menu/add_display', $data);
                    $this->load->view('admin/partials/footer');
                }
                ...
                $formArray['about'] = $this->input->post('about');
                $formArray['price'] = $this->input->post('price');
                $formArray['di_id'] = $this->input->post('rname');
    
                $this->Menu_model->createc($formArray);
            ...
            $store_data['stores']= $store;
            $this->load->view('admin/partials/header');
            $this->load->view('admin/menu/add_display', $store_data);
            $this->load->view('admin/partials/footer');
        }
    ...
    public function edit_display($id) {
        $this->load->model('Menu_model');
        $dish = $this->Menu_model->getSingleDisplay($id);
        ...
        if($this->form_validation->run() == true) {
            ...
            if(!empty($_FILES['image']['name'])){
                ...
                if($this->upload->do_upload('image')) {
                    ...
                    $data = $this->upload->data();
                    //resizing image
                    resizeImage($config['upload_path'].$data['file_name'], $config['upload_path'].'display/'.$data['file_name'], 300,270);
                    ...
                    if (file_exists('./public/uploads/dishesh/display/'.$dish['img'])) {
                        unlink('./public/uploads/dishesh/display/'.$dish['img']);
                    }
                    ...
                    $data['stores'] = $store;
                    $this->load->view('admin/partials/header');
                    $this->load->view('admin/menu/edit_display', $data);
                    $this->load->view('admin/partials/footer');
                }
                ...
                $this->Menu_model->update_display($id, $formArray);
    ...
    public function delete_display($id){
        ...
        if (file_exists('./public/uploads/dishesh/display'.$dish['image'])) {
            unlink('./public/uploads/dishesh/display'.$dish['image']);
        }

        if(file_exists('./public/uploads/dishesh/thumb/'.$dish['image'])) {
            unlink('./public/uploads/dishesh/thumb/'.$dish['image']);
        }

        $this->Menu_model->delete_display($id);
